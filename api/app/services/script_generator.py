...
class ScriptGenerator:
    ...
    @staticmethod
    def create_update_prompt(instructions: str, script: str, selected_tables: list[Table]) -> str:
        table_schemas = [table.schema_sql for table in selected_tables]
        return f'''Your task is to iterate the provided Python script for creating fake data to better match the user query and schemas provided.
        The script should use appropriate libraries (e.g., Faker) to generate realistic data. Pay special attention to foreign key relationships and ensure that the data generated for these relationships references previously generated data in the appropriate manner.
    ...
    def update(self, prompt: str, script: str, selected_tables: list[Table]) -> ScriptGenerationResponse:
        try:
            response = self.client.beta.chat.completions.parse(
                model="gpt-4o-2024-08-06",
                messages=[
                    {"role": "system", "content": self.create_system_prompt()},
                    {"role": "user", "content": self.create_update_prompt(prompt, script, selected_tables)}
                ],
                response_format=ScriptGenerationResponse
            )
            return response.choices[0].message.parsed
        except Exception as e:
            logger.error(f"Error updating python script: {str(e)}")
            raise

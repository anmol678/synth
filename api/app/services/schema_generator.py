...
class SchemaGenerator:
    ...
    def create_system_prompt() -> str:
        ...
        You are encouraged to use a step by step approach in the provided scratchpad to plan and reason. Finally, you must provide a list of table names and their respective PostgreSQL schemas.
        You must always respond in valid JSON format.
        '''

    @staticmethod
    def create_prompt(query: str) -> str:
        return f'''Your task is to break down the user query to generate perfectly defined PostgreSQL schemas for one or more tables, including any dependencies or foreign key relationships between them.
    ...
    @staticmethod
    def create_update_prompt(query: str, tables: list[Table]) -> str:
        return f'''Your task is to iterate the provided PostgreSQL schemas to better match the user query.
        
        USER QUERY:
        {query}
        
        CURRENT TABLES AND SCHEMAS:
        {chr(10).join([table.schema_sql for table in tables])}
        '''
    ...
    def update(self, prompt: str, tables: list[Table]) -> SchemaResponse:
        try:
            response = self.client.beta.chat.completions.parse(
                model="gpt-4o-2024-08-06",
                messages=[
                    {"role": "system", "content": self.create_system_prompt()},
                    {"role": "user", "content": self.create_update_prompt(prompt, tables)}
                ],
                response_format=SchemaResponse
            )
            return response.choices[0].message.parsed
